...
type userEventLogWriter interface {
	pushUserEventLog(ctx context.Context, template EventLogReq, getData func([]any) map[string]map[string]any) (err error)
}

func (u *userEventLogService) pushUserEventLog(ctx context.Context, template EventLogReq, getData func([]any) map[string]map[string]any) (err error) {
	...
	if u.kafka == nil {
		err = errors.New("kafka is not initialized")
		return err
	}
	...
	for i := 0; i < len(template.IDs); i += 100 {
		...
		if len(templateIDs) > 0 {
			...
			if len(data) > 0 {
				...
				err = u.kafka.Push("v1.logging_service.user_event_log.write_bulk", event)
				if err != nil {
