...
func newZapLogger(level LogLevel, timeFormat string) *zap.Logger {
	...
	skipCaller := zap.AddCallerSkip(2)

	logger := zap.New(core, caller, skipCaller)
...
func GetZapLogger(l Logger) *zap.Logger {
	...
	case *loggerImpl:
		return v.zap
	default:
		//l.Info("logger.WithFields: invalid logger type, creating a new zap logger", String("level", string(LevelInfo)), String("time_format", time.RFC3339))
		return newZapLogger(LevelInfo, time.RFC3339)
	}
}
