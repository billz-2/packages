...
// err == nil означает: спросили про всех. Кого нет в map — того нет в BILLZ (404).
```

**Всё или ничего.** Любой сбой дозапроса (5xx, таймаут, обрыв) возвращается
ошибкой, а `users` при этом `nil` — частичной пачки не бывает, даже если часть
id уже нашлась в кеше. Иначе вызывающий не отличит «пользователя нет» от «не
смогли спросить»: и то и другое выглядит как отсутствующий ключ в map, и он
молча раздаст дефолтный язык тем, у кого язык на самом деле задан. Ненайденные
(404) сбоем не считаются — таких id в map просто нет.
...
| `GetUsersByIDs`, любой дозапрос упал | ошибка и `nil` map — всё или ничего |
| `GetUsersByIDs`, часть id отдала 404 | успех; ненайденных в map просто нет |
